...
(function () {
  ...
  function appendMessage(peer, text) {
    ...
    who.className = "who " + peer;
    who.textContent = peer + ":";
  ...
  // ── Reroll: register BEFORE recorder.start() so the click is a
  // plain native listener, invisible to the recorder. If we captured
  // reroll clicks, they'd broadcast and chain-reload all tabs.
  ...
  // Disable subsystems we're not using — keeps the fetch patch from
  // installing a pending-oracle on globalThis.fetch.
  ...
  ch.onmessage = (e) => {
    ...
    if (from === me) {
      // BroadcastChannel doesn't echo to the posting context, so in
      // practice this branch never hits for same-origin same-browser.
      // Kept for other transports that do echo.
      echoes += ops.length;
  ...
  // ── App handlers registered AFTER recorder.start() so the wrapper
  // records them. Until a listener is attached to a subsystem, the
  // recorder has nothing to intercept — the browser fires events to
  // its own internal listener list and we never see them.

  // Keystrokes in the input → input events → detail.value replicates
  // the field on the other peer's screen. The empty handler exists
  // purely so the recorder's wrapped addEventListener has something
  // to see.
  input.addEventListener("input", () => {});

  // Submit: handleSubmit reads input.value (which the remote peer's
  // input events already populated via applyEvent) and appends.
  ...
  systemMessage("joined as " + me + " — open a second tab with #alice, #bob, #carol, or #dave");
  refreshStats();
})();
  </script>
</body>
